...
public class AreaStatistic {
    ...
    public static void main(String[] args) throws Exception {
        ...
        sparkConf.setAppName("AreaStatistic");
...
//        String hdfsName = (String)parameters.get("hdfsname");
        String hdfsName = CommonSettings.DEFAULT_HDFS_PATH;
        String ccFilter = (String)parameters.get("categorycode");
        String rowkeyArr = (String)parameters.get("shapepath");
        Map<String,Integer> ccFilterMap = edu.zju.gis.spark.Utils.ccFliterFormat(ccFilter);
        Map<String,Integer> provinceFilter = new HashMap<String,Integer>();
        Map<String,Integer> cityFilter = new HashMap<String,Integer>();
        Map<String,Integer> countyFilter = new HashMap<String,Integer>();
        Utils.regionCodeFilterFormat(rowkeyArr.split(","),provinceFilter,cityFilter,countyFilter);
        Boolean IsCountry=false;
        if(rowkeyArr.equals("0000"))
            IsCountry = true;
        ...
        final Broadcast<Map<String, Integer>> provinceFilterBC = ctx.broadcast(provinceFilter);
        final Broadcast<Map<String, Integer>> cityFilterBC = ctx.broadcast(cityFilter);
        final Broadcast<Map<String, Integer>> countyFilterBC = ctx.broadcast(countyFilter);
        final Broadcast<Map<String, Integer>> ccFilterBC = ctx.broadcast(ccFilterMap);
        final Broadcast<Boolean> IsCountryBC = ctx.broadcast(IsCountry);
...
//                    System.out.println(regionID.substring(0,2));
                    ...
                    if (countyFilterBC.getValue().containsKey(regionCode)
                            ||cityFilterBC.getValue().containsKey(regionCode)
                            ||provinceFilterBC.getValue().containsKey(regionCode)
                            ||IsCountryBC.value() ) {
